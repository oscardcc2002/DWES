...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $conn = new mysqli(HOST, MYSQL_USER, MYSQL_PASSWORD, "EMPRESA");
    ...
    switch ($tipoConsulta) {
        ...
        case 'ListadoCompras':
            ...
            $consulta = "SELECT c.*, cl.NOMBRE AS NOMBRE_CLIENTE, cl.APELLIDOS AS APELLIDOS_CLIENTE, p.NOMBRE AS NOMBRE_PRODUCTO, pr.NOMBRE AS NOMBRE_PROVEEDOR FROM COMPRA c
            JOIN CLIENTE cl ON c.CLIENTE = cl.ID_CLI
            JOIN PRODUCTO p ON c.PRODUCTO = p.COD_PROD
            JOIN PROVEEDOR pr ON p.PROVEEDOR = pr.ID_PROV
            ORDER BY cl.DNI, p.COD_PROD";
            $result = $conn->query($consulta);
            break;
    ...
    if (isset($consulta)) {
        ...
        // Libera los recursos del resultado y cierra la conexión
        $result->close();
        $conn->close();
    }
